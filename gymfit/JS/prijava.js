...
function prijava() 
{
	...
	xhr.open("POST", "http://192.168.64.109/gymfitAPI/prijava.php", true); // vpisemo URL mesta za avtentikacijo v API-ju zaledja
	...
	xhr.onreadystatechange = function () {
	   if (xhr.readyState === 4) 
	   {
			if(xhr.status === 200) 
			{
				...
				window.localStorage.setItem("user_level", response["level"]);
